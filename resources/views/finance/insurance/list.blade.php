				...
				<div class="col-xl-12 col-lg-12 ">
					...
					<div class="card card-transparent">
						<div class="card-block p-t-0">
							<h3 class='m-t-0'>Senarai Pembayaran Premium Insuran</h3>
							<p class="small hint-text m-t-5">
								Pengurusan Permohonan Pembayaran Premium Insuran boleh dilakukan melalui jadual di bawah
							</p>
						</div>
					</div>
					<!-- END card -->
			...
			<div class="card-title">
				@if(auth()->user()->hasAnyRole(['ks']))
                    @if(auth()->user()->entity->tenures->count() > 1)
                        ...
                        @else
                            <a id="" class="btn btn-success btn-cons text-capitalize" href="{{ route('insurance') }}"><i class="fa fa-plus m-r-5"></i> Permohonan</a>
                        @endif
                    @else
                        <a id="" class="btn btn-success btn-cons text-capitalize" href="{{ route('insurance') }}"><i class="fa fa-plus m-r-5"></i> Permohonan</a>
                    @endif
		...
		<div class="modal-content-wrapper">
			<div class="modal-content">
				<div class="modal-header clearfix text-left" style="background-color: #f3f3f3;">
					...
					<h5>Maklumat <span class="semi-bold">Permohonan Pembayaran Insuran</span></h5>
				...
				<div class="modal-footer" style="background-color: #f3f3f3;">
                    ...
                    <a href="{{ route('insurance') }}" class="btn btn-primary mt-4"><i class="fa fa-edit mr-1 text-capitalize"></i> Kemaskini Borang</a>
                    @endif
...
function process(id) {
    $("#modal-div").load("{{ route('insurance') }}/"+id+"/process/result");
}

function receive(id) {
    $("#modal-div").load("{{ route('insurance') }}/"+id+"/process/document-receive");
}

function query(id) {
    $("#modal-div").load("{{ route('insurance') }}/"+id+"/process/query");
}

function recommend(id) {
    $("#modal-div").load("{{ route('insurance') }}/"+id+"/process/recommend");
}

function status(id) {
    $("#modal-div").load("{{ route('insurance') }}/"+id+"/process/status");
}
