		...
		<div class="card-block">
			<div>
                <form id="form-formu" class="form-horizontal" role="form" autocomplete="off" method="post" action="#">
                    ...
                    @include('finance.insurance.formu.examiner.index')
...
function calc() {
    ...
    if($("input[name=total_supporting]").val() + $("input[name=total_against]").val() > $("input[name=total_voters]").val() ) {
        $("#percentage").html('');
        $("input[name=total_supporting]").val(0);
        $("input[name=total_against]").val(0);
        
        swal('Opps!', 'Jumlah undi tidak boleh melebihi jumlah anggota yang layak mengundi.', 'error');
        return;
    }
