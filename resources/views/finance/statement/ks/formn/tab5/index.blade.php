...
<div class=" container-fluid container-fixed-lg bg-white">
	...
	<div class="card card-transparent">
		<div class="col-md-12 text-center p-t-20">
			...
			II. PEGAWAI-PEGAWAI YANG DILANTIK
		</div>
		<div class="card-header px-0">
			...
			<div class="pull-right">
				<div class="col-xs-12">
					<input type="text" id="appointed-search-table" class="form-control search-table pull-right" placeholder="Carian">
				</div>
			</div>
			<div class="clearfix"></div>
		</div>
		<div class="card-block">
			<table class="table table-hover table-responsive-block" id="table-appointed">
				<thead>
					<tr>
						<th class="fit">Bil.</th>
						<th>Nama</th>
						<th class="fit">Jawatan</th>
						<th class="fit">Tarikh Perlantikan</th>
						<th class="fit">Tindakan</th>
					</tr>
				</thead>
			</table>
			<button id="" class="btn btn-primary btn-cons" type="button" onclick="addAppointed()"><i class="fa fa-plus m-r-5"></i> Tambah Pegawai</button>
		</div>
	</div>
	<!-- END card -->
...
<!-- END CONTAINER FLUID -->
@push('modal')
<div class="modal fade" id="modal-addAppointed" tabindex="-1" role="dialog" aria-hidden="false">
		...
		<div class="modal-content-wrapper">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="form-add-appointed" role="form" method="post" action="{{ route('formn.appointed', $statement->id) }}">
							...
							<div class="row">
								<div class="col-md-12">
									<div class="form-group form-group-default form-group-default-custom form-group-default-select2 required">
										...
										<select id="" name="designation_id" class="full-width autoscroll" data-init-plugin="select2" required>
					...
					<div class="row">
						<div class="col-md-12 p-t-10">
							<button type="button" class="btn btn-info m-t-5 pull-right" onclick="submitForm('form-add-appointed')"><i class="fa fa-check mr-1"></i> Simpan</button>
						</div>
					</div>
				</div>
			</div>
...
<script type="text/javascript">
	...
	var table10 = $('#table-appointed');

	var settings10 = {
	    "processing": true,
	    "serverSide": true,
	    "deferRender": true,
	    "ajax": "{{ route('formn.appointed', $statement->id) }}",
	    "columns": [
	...
	table10.dataTable(settings10);

	// search box for table
	$('#appointed-search-table').keyup(function() {
	    table10.fnFilter($(this).val());
	});

	function addAppointed() {
	    $('#modal-addAppointed').modal('show');
	    $('.modal form').trigger("reset");
	    $('.modal form').validate();
	}

	function editAppointed(id) {
		$("#modal-div").load("{{ route('formn.appointed', $statement->id) }}/"+id);
	}

	$("#form-add-appointed").submit(function(e) {
	    e.preventDefault();
	    var form = $(this);
	    ...
	    $.ajax({
	        ...
	        success: function(data) {
	            ...
	            $("#modal-addAppointed").modal("hide");
	            table10.api().ajax.reload(null, false);
	        }
	    });
	});

	function removAppointed(id) {
	    swal({
	        title: "Padam Data",
	    ...
	    .then((confirm) => {
	        if (confirm) {
	            $.ajax({
	                url: '{{ route('formn.appointed', $statement->id) }}/'+id,
	                method: 'delete',
	                dataType: 'json',
	                async: true,
	                contentType: false,
