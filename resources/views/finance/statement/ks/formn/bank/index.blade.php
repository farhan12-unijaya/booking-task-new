<div class="form-group row">
	<label>Dalam Bank (nyatakan)</label>
    ...
    <button onclick="addBank()" class="btn btn-primary btn-sm btn-cons" type="button"><i class="fa fa-plus m-r-5"></i> Tambah Dalam Bank</button>
</div>
@push('modal')
		...
		<div class="modal-content-wrapper">
			<div class="modal-content">
				<div class="modal-header clearfix text-left">
					...
					<h5>Maklumat <span class="semi-bold">Dalam Bank</span></h5>
				</div>
				<div class="modal-body">
					<form id="form-add-bank" role="form" method="post" action="{{ route('formn.bank', $statement->id) }}">
...
var table8 = $('#table-bank');

var settings8 = {
    "processing": true,
    "serverSide": true,
    "deferRender": true,
    "ajax": "{{ route('formn.bank', $statement->id) }}",
    "columns": [
...
function editBank(id) {
    $("#modal-div").load("{{ route('formn.bank', $statement->id) }}/"+id);
}
...
function removeBank(id) {
    ...
    .then((confirm) => {
        if (confirm) {
            $.ajax({
                url: '{{ route('formn.bank', $statement->id) }}/'+id,
                method: 'delete',
                dataType: 'json',
                async: true,
                contentType: false,
