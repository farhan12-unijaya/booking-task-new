                ...
                <div class="col-xl-12 col-lg-12 ">
                    ...
                    <div class="card card-transparent">
                        <div class="card-block p-t-0">
                            <h3 class='m-t-0'>Senarai <span class="semi-bold">Penyata Tahunan Kesatuan - Kesatuan Sekerja</span></h3>
                            <p class="small hint-text m-t-5">
                                Pengurusan permohonan Penyata Tahunan boleh dilakukan melalui jadual di bawah
                            </p>
                        </div>
                    </div>
                    <!-- END card -->
            ...
            <div class="card-title">
                @if(auth()->user()->hasAnyRole(['ks']))
                    @if(auth()->user()->entity->tenures->count() > 1)
                        ...
                        @else
                            <a id="" class="btn btn-success btn-cons text-capitalize" href="{{ route('statement.ks') }}"><i class="fa fa-plus m-r-5"></i> Permohonan</a>
                        @endif
                    @else
                        <a id="" class="btn btn-success btn-cons text-capitalize" href="{{ route('statement.ks') }}"><i class="fa fa-plus m-r-5"></i> Permohonan</a>
                    @endif
        ...
        <div class="modal-content-wrapper">
            <div class="modal-content">
                ...
                <div class="modal-footer" style="background-color: #f3f3f3;">
                    ...
                    <a href="{{ route('statement.ks') }}" class="btn btn-primary mt-4"><i class="fa fa-edit mr-1 text-capitalize"></i> Kemaskini Borang</a>
                    @endif
                    @if(auth()->user()->hasAnyRole(['ppw','pw', 'pphq', 'pkpp','pkpg', 'ppp','kpks']))
                    <button onclick="queryData(1)" class="btn btn-warning mt-4"><i class="fa fa-question mr-1"></i> Kuiri</button>
                    @endif
                     @if(auth()->user()->hasAnyRole(['ppw','pw', 'pphq', 'pkpg', 'pkpp', 'ppp']))
...
function process(id) {
    $("#modal-div").load("{{ route('statement.ks') }}/"+id+"/process/result");
}

function receive(id) {
    $("#modal-div").load("{{ route('statement.ks') }}/"+id+"/process/document-receive");
}

function query(id) {
    $("#modal-div").load("{{ route('statement.ks') }}/"+id+"/process/query");
}

function recommend(id) {
    $("#modal-div").load("{{ route('statement.ks') }}/"+id+"/process/recommend");
}

function status(id) {
    $("#modal-div").load("{{ route('statement.ks') }}/"+id+"/process/status");
}
