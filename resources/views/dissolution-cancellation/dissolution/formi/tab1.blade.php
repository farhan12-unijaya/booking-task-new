  <div class=" container-fluid container-fixed-lg bg-white">
    ...
    <div class="card card-transparent">
        <div class="card-block">
            <div>
                <form id="form-formi" class="form-horizontal" role="form" autocomplete="off" method="post" action="#">

                    @component('components.bs.label', [
                        'label' => 'Nama Kesatuan Sekerja',
                    ])
                    {{ $formi->tenure->entity->name }}
                    @endcomponent

                    @component('components.bs.label', [
                        'label' => 'No. Pendaftaran',
                    ])
                    {{ $formi->tenure->entity->registration_no }}
                    @endcomponent

                    @component('components.bs.label', [
                        'label' => 'Alamat Ibu Pejabat',
                    ])
                    {!! $formi->address->address1.
                    ($formi->address->address2 ? ',<br>'.$formi->address->address2 : '').
                    ($formi->address->address3 ? ',<br>'.$formi->address->address3 : '').
                    ',<br>'.
                    $formi->address->postcode.' '.
                    ($formi->address->district ? $formi->address->district->name : '').', '.
                    ($formi->address->state ? $formi->address->state->name : '') !!}
                    @endcomponent
                    ...
                    @include('components.bs.date', [
                        ...
                        'mode' => 'required',
                        'value' =>  $formi->applied_at ? date('d/m/Y', strtotime($formi->applied_at)) : '',
                    ])

                    @include('components.bs.date', [
                        'name' => 'concluded_at',
                        'label' => 'Tarikh Undi Rahsia',
                        'mode' => 'required',
                        'value' =>  $formi->concluded_at ? date('d/m/Y', strtotime($formi->concluded_at)) : '',
                    ])
                    ...
                    @include('components.bs.date', [
                        ...
                        'mode' => 'required',
                        'value' =>  $formi->resolved_at ? date('d/m/Y', strtotime($formi->resolved_at)) : '',
                    ])

                   	@component('components.bs.label', [
                        'label' => 'Nama Setiausaha Penaja',
                    ])
                    {{ $formi->secretary->name }}
                    @endcomponent
