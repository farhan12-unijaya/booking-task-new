		...
		<div class="modal-content-wrapper">
			<div class="modal-content">
				...
				<div class="modal-body">
                    <form  id="form-add-member" role="form" method="post" action="{{ route('forme.member', [$dissolution->id]) }}">
...
var settings = {
	...
	"serverSide": true,
	"deferRender": true,
	"ajax": "{{ route('forme.member', [$dissolution->id]) }}",
	"columns": [
...
function editMember(id) {
	$("#modal-div").load("{{ route('forme.member', [$dissolution->id]) }}/"+id);
}
...
function removeMember(id) {
	...
	.then((confirm) => {
		if (confirm) {
			$.ajax({
				url: '{{ route('forme.member', [$dissolution->id]) }}/'+id,
				method: 'delete',
				dataType: 'json',
				async: true,
				contentType: false,
