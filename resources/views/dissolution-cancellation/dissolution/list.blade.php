                ...
                <div class="col-xl-12 col-lg-12 ">
                    ...
                    <div class="card card-transparent">
                        <div class="card-block p-t-0">
                            <h3 class='m-t-0'>Senarai <span class="semi-bold">Pembubaran Kesatuan Sekerja</span></h3>
                            <p class="small hint-text m-t-5">
                                Pengurusan permohonan Pembubaran Kesatuan Sekerja boleh dilakukan melalui jadual di bawah.
                            </p>
                        </div>
                    </div>
                    <!-- END card -->
    ...
    <div class="card card-transparent">
        <div class="card-header px-0">
            <div class="card-title">
                ...
                <button onclick="location.href='{{ route('dissolution') }}'" class="btn btn-success btn-cons"><i class="fa fa-plus m-r-5"></i> Permohonan</button>
                @endif
            </div>
        ...
        <div class="card-block table-responsive">
            <table class="table table-hover" id="table-dissolution">
                <thead>
                    <tr>
                        <th class="fit">Bil.</th>
                        <th>Nama Kesatuan</th>
        ...
        <div class="modal-content-wrapper">
            <div class="modal-content">
                ...
                <div class="modal-footer" style="background-color: #f3f3f3;">
                    ...
                    @endif
                    @if(auth()->user()->hasAnyRole(['ppw','pphq']))
                    <a href="{{ route('dissolution') }}" class="btn btn-primary mt-4"><i class="fa fa-edit mr-1 text-capitalize"></i> Kemaskini</a>
                    @endif
                    @if(auth()->user()->hasAnyRole(['pw', 'ppw', 'pphq', 'pkpg', 'pkpp','kpks']))
                    <button onclick="queryData(1)" class="btn btn-warning mt-4"><i class="fa fa-question mr-1"></i> Kuiri</button>
                    @endif
                    @if(auth()->user()->hasAnyRole(['pw', 'ppw', 'pphq', 'pkpg','pkpp']))
...
function process(id) {
    $("#modal-div").load("{{ route('dissolution') }}/"+id+"/process/result");
}

function receive(id) {
    $("#modal-div").load("{{ route('dissolution') }}/"+id+"/process/document-receive");
}

function query(id) {
    $("#modal-div").load("{{ route('dissolution') }}/"+id+"/process/query");
}

function recommend(id) {
    $("#modal-div").load("{{ route('dissolution') }}/"+id+"/process/recommend");
}
