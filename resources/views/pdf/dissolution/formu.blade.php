...
<body class="letter">
	...
	<div class="dotted line" style="padding-bottom: 5px; text-align: left;">
		<span class="a">Nama kesatuan sekerja berdaftar</span> {{ $formieu->forml->tenure->entity->name }}
	</div>
	<div class="dotted line" style="padding-bottom: 5px; text-align: left;">
		<span class="a">Alamat ibu pejabat berdaftar </span> 
		@if($formieu->forml->address)
			{{ $formieu->forml->address->address1 ? $formieu->forml->address->address1.',' : '' }}
			{{ $formieu->forml->address->address2 ? $formieu->forml->address->address2.',' : '' }}
			{{ $formieu->forml->address->address3 ? $formieu->forml->address->address3.',' : '' }}
			{{ $formieu->forml->address->postcode ? $formieu->forml->address->postcode : '' }}
			{{ $formieu->forml->address->district ? $formieu->forml->address->district->name.',' : '' }}
			{{ $formieu->forml->address->state ? $formieu->forml->address->state->name.'.' : '' }}
		@endif
	</div>
	<div class="dotted line" style="padding-bottom: 5px; text-align: left;">
	    ...
	    <tr>
	  	...
	  	<tr class="top">
	  		...
	  		<td  colspan="3" class="justify">
	  			<span class="dotted">{{ $formieu->forml->formu->setting }}</span>
	  		</td>
	  	</tr>
	  	<tr>
	  		<td colspan="4" class="justify"> 
	  			<div style="text-indent: 90px">Kami mengesahkan bahawa suatu undi sulit telah diambil dengan sempurna pada ketetapan di atas/pemilihan pegawai-pegawai* pada <span class="dotted">{{ strftime('%e', strtotime($formieu->forml->formu->voted_at)) }}</span> haribulan <span class="dotted">{{ strftime('%B', strtotime($formieu->forml->formu->voted_at)).' '.strftime('%Y', strtotime($formieu->forml->formu->voted_at)) }}</span>.Keputusan adalah seperti berikut.</div>
	  		</td>
	  	</tr>
	  	<tr>
			<td></td>
			<td colspan="4">
				Jumlah bilangan anggota-anggota yang berhak untuk mengundi <span class="dotted" style="width: 100px">{{ $formieu->forml->formu->total_voters }}</span> <br>
				Jumlah bilangan kertas undi yang dikeluarkan <span class="dotted" style="width: 100px">{{ $formieu->forml->formu->total_voters }}</span>  <br>
				Jumlah bilangan kertas undi yang dipulangkan <span class="dotted" style="width: 100px">{{ $formieu->forml->formu->total_slips }}</span>  <br>
			</td>
		</tr>
		<tr>
			...
			<td colspan="3">
				...
				Undi-undi menyokong <span class="dotted" style="width: 100px">{{ $formieu->forml->formu->total_supporting }}</span> <br>
				Undi-undi menentang <span class="dotted" style="width: 100px">{{ $formieu->forml->formu->total_against }}</span>
			</td>
		</tr>
		<tr>
			...
			<td colspan="4">
				...
				<table>
					...
					<tr>
						...
						<td>
							@foreach($formieu->forml->formu->examiners as $index => $examiner)
							<div class="dotted line" style="padding-bottom: 5px; text-align: left;">
					...
					<tr>
						<td>
							<div class="line" style="padding-bottom: 5px;margin-left: 0px; text-align: left;">
