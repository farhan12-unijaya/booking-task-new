	...
	<div class="row mt-5">
		<div class="col-md-12">
			<ul class="pager wizard no-style">
				@if($formw->created_by_user_id == auth()->id())
				<li class="submit">
		...
		<div class="modal-content-wrapper">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form  id="form-add-officer" role="form" method="post" action="{{ route('formw.officer', $formw->id) }}">
...
<script>
	...
	var table3 = $('#table-officer');

	var settings3 = {
	    "processing": true,
	    "serverSide": true,
	    "deferRender": true,
	    "ajax": "{{ route('formw.officer', $formw->id) }}",
	    "columns": [
	...
	function editOfficer(id) {
		$("#modal-div").load("{{ route('formw.officer', $formw->id) }}/"+id);
	}
	...
	function removeOfficer(id) {
	    ...
	    .then((confirm) => {
	        if (confirm) {
	            $.ajax({
	                url: '{{ route('formw.officer', $formw->id) }}/'+id,
	                method: 'delete',
	                dataType: 'json',
	                async: true,
	                contentType: false,
