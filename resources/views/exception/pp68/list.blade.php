                ...
                <div class="col-xl-12 col-lg-12 ">
                    ...
                    <div class="card card-transparent">
                        <div class="card-block p-t-0">
                            <h3 class='m-t-0'>Senarai <span class="semi-bold">Permohonan Pengecualian Peraturan 68</span></h3>
                            <p class="small hint-text m-t-5">
                                Pengurusan permohonan Permohonan Pengecualian Peraturan 68 boleh dilakukan melalui jadual di bawah.
                            </p>
                        </div>
                    </div>
                    <!-- END card -->
...
<div class=" container-fluid container-fixed-lg bg-white">
    ...
    <div class="card card-transparent">
            ...
            <div class="card-title">
                @if(auth()->user()->hasAnyRole(['ks']))
                    @if(auth()->user()->entity->tenures->count() > 1)
                        ...
                        @else
                            <a id="" class="btn btn-success btn-cons text-capitalize" href="{{ route('pp68') }}"><i class="fa fa-plus m-r-5"></i> Pengecualian</a>
                        @endif
                    @else
                        <a id="" class="btn btn-success btn-cons text-capitalize" href="{{ route('pp68') }}"><i class="fa fa-plus m-r-5"></i> Pengecualian</a>
                    @endif
        ...
        <div class="card-block table-responsive">
            <table class="table table-hover" id="table-pp68">
                <thead>
                    <tr>
                        <th class="fit">Bil.</th>
                        <th>Nama Kesatuan</th>
        ...
        <div class="modal-content-wrapper">
            <div class="modal-content">
                ...
                <div class="modal-footer" style="background-color: #f3f3f3;">
                    ...
                    <a href="{{ route('pp68') }}" class="btn btn-primary mt-4"><i class="fa fa-edit mr-1 text-capitalize"></i> Kemaskini Borang</a>
                    @endif
...
function process(id) {
    $("#modal-div").load("{{ route('pp68') }}/"+id+"/process/result");
}

function receive(id) {
    $("#modal-div").load("{{ route('pp68') }}/"+id+"/process/document-receive");
}

function query(id) {
    $("#modal-div").load("{{ route('pp68') }}/"+id+"/process/query");
}

function recommend(id) {
    $("#modal-div").load("{{ route('pp68') }}/"+id+"/process/recommend");
}

function status(id) {
    $("#modal-div").load("{{ route('pp68') }}/"+id+"/process/status");
}
</script>
@endpush
