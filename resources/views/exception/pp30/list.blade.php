                ...
                <div class="col-xl-12 col-lg-12 ">
                    ...
                    <div class="card card-transparent">
                        <div class="card-block p-t-0">
                            <h3 class='m-t-0'>Senarai <span class="semi-bold">Permohonan Pengecualian Seksyen 30(b)</span></h3>
                            <p class="small hint-text m-t-5">
                                Pengurusan permohonan Permohonan Pengecualian Seksyen 30(b) boleh dilakukan melalui jadual di bawah.
                            </p>
                        </div>
                    </div>
                    <!-- END card -->
...
<div class=" container-fluid container-fixed-lg bg-white">
    ...
    <div class="card card-transparent">
                ...
                @if(auth()->user()->hasAnyRole(['ks']))
                    @if(auth()->user()->entity->tenures->count() > 1)
                        ...
                        @if($tenure->formlu()->where('filing_status_id', 9)->count() == 0)
                            <button onclick="swal('Harap Maaf!', 'Sila mendaftar borang LU bagi penggal ini terlebih dahulu atau hubungi admin.', 'error')" class="btn btn-success btn-cons"><i class="fa fa-plus m-r-5"></i> Pengecualian</button>
                        @else
                            <a id="" class="btn btn-success btn-cons text-capitalize" href="{{ route('pp30') }}"><i class="fa fa-plus m-r-5"></i> Pengecualian</a>
                        @endif
                    @else
                        <a id="" class="btn btn-success btn-cons text-capitalize" href="{{ route('pp30') }}"><i class="fa fa-plus m-r-5"></i> Pengecualian</a>
                    @endif
        ...
        <div class="card-block table-responsive">
            <table class="table table-hover" id="table-pp30">
                <thead>
                    <tr>
                        <th class="fit">Bil.</th>
                        <th>Nama Kesatuan</th>
        ...
        <div class="modal-content-wrapper">
            <div class="modal-content">
                ...
                <div class="modal-footer" style="background-color: #f3f3f3;">
                    ...
                    @endif
                    @if(auth()->user()->hasAnyRole(['ks', 'pphq']))
                    <a href="{{ route('pp30') }}" class="btn btn-primary mt-4"><i class="fa fa-edit mr-1 text-capitalize"></i> Kemaskini Borang</a>
                    @endif
...
function process(id) {
    $("#modal-div").load("{{ route('pp30') }}/"+id+"/process/result");
}

function minister(id) {
    $("#modal-div").load("{{ route('pp30') }}/"+id+"/process/minister");
}

function receive(id) {
    $("#modal-div").load("{{ route('pp30') }}/"+id+"/process/document-receive");
}

function query(id) {
    $("#modal-div").load("{{ route('pp30') }}/"+id+"/process/query");
}

function recommend(id) {
    $("#modal-div").load("{{ route('pp30') }}/"+id+"/process/recommend");
}

function status(id) {
    $("#modal-div").load("{{ route('pp30') }}/"+id+"/process/status");
}
</script>
@endpush
