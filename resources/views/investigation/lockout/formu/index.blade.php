		...
		<div class="card-block">
			...
			<div>
                <form id="form-formu" class="form-horizontal" role="form" autocomplete="off" method="post" action="#">
					...
					{!! $formu->address->address1.
					($formu->address->address2 ? ',<br>'.$formu->address->address2 : '').
					($formu->address->address3 ? ',<br>'.$formu->address->address3 : '').
					',<br>'.
					$formu->address->postcode.' '.
					($formu->address->district ? $formu->address->district->name : '').', '.
					($formu->address->state ? $formu->address->state->name : '') !!}
					@endcomponent
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <select id="branch_id" name="branch_id" class="full-width autoscroll" data-init-plugin="select2" required>
                                <option value="-1" selected>Induk - {{ auth()->user()->entity->name }}</option>
                                @foreach(auth()->user()->entity->branches as $branch)
    				...
    				@component('components.bs.label', [
    					'name' => 'result',
    					'label' => 'Maka Ketetapan Adalah',
    				])
    				<span id="result"></span>
    				@endcomponent


                    @include('investigation.lockout.formu.examiner.index')
...
function calc() {
    ...
    $("#result").html( supporting/(supporting+against) >= 2/3 ? "<span class='badge badge-success'>Menang</span>" : "<span class='badge badge-danger'>Kalah</span>" );

    if(supporting/(supporting+against) >= 2/3)
