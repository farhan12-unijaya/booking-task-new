				...
				<div class="col-xl-12 col-lg-12 ">
					...
					<div class="card card-transparent">
						<div class="card-block p-t-0">
							<h3 class='m-t-0'>Senarai <span class="semi-bold">Aduan</span></h3>
							<p class="small hint-text m-t-5">
								Pengurusan permohonan aduan boleh dilakukan melalui jadual di bawah
							</p>
						</div>
					</div>
					<!-- END card -->
	...
	<div class="card card-transparent">
		<div class="card-header px-0">
			<div class="card-title">
				...
				<a id="" class="btn btn-success btn-cons text-capitalize" href="{{ route('investigation.complaint') }}"><i class="fa fa-plus m-r-5"></i> Aduan</a>
				@endif
			</div>
		...
		<div class="card-block table-responsive">
			<table class="table table-hover " id="table-complaints">
				<thead>
					<tr>
						<th class="fit">Bil.</th>
						<th>Aduan Oleh</th>
						<th>Tajuk Surat Aduan</th>
						<th>Pihak Yang Diadu</th>
						<th>Tarikh Aduan</th>
						<th>Status</th>
		...
		<div class="modal-content-wrapper">
			<div class="modal-content">
				...
				<div class="modal-footer" style="background-color: #f3f3f3;">
                    ...
                    <a href="{{ route('investigation.complaint') }}" class="btn btn-primary mt-4"><i class="fa fa-edit mr-1 text-capitalize"></i> Kemaskini Borang</a>
                    @endif
                    @if(auth()->user()->hasAnyRole(['ptw', 'pthq']))
                    <button onclick="statusData(1)" class="btn btn-primary mt-4"><i class="fa fa-edit mr-1"></i> Kemaskini Status</button>
                    @endif
                    ...
                    @endif
                    @if(auth()->user()->hasAnyRole(['pw', 'pkpp', 'kpks']))
...
function process(id) {
    $("#modal-div").load("{{ route('investigation.complaint') }}/"+id+"/process/result");
}

function receive(id) {
    $("#modal-div").load("{{ route('investigation.complaint') }}/"+id+"/process/document-receive");
}

function query(id) {
    $("#modal-div").load("{{ route('investigation.complaint') }}/"+id+"/process/query");
}

function recommend(id) {
    $("#modal-div").load("{{ route('investigation.complaint') }}/"+id+"/process/recommend");
}

function status(id) {
    $("#modal-div").load("{{ route('investigation.complaint') }}/"+id+"/process/status");
}
