				...
				<div class="col-xl-12 col-lg-12 ">
					...
					<div class="card card-transparent">
						<div class="card-block p-t-0">
							<h3 class='m-t-0'>Senarai <span class="semi-bold">Mogok</span></h3>
							<p class="small hint-text m-t-5">
								Pengurusan permohonan mogok boleh dilakukan melalui jadual di bawah
							</p>
						</div>
					</div>
					<!-- END card -->
	...
	<div class="card card-transparent">
		<div class="card-header px-0">
			<div class="card-title">
				...
				<a id="" class="btn btn-success btn-cons text-capitalize" href="{{ route('strike') }}"><i class="fa fa-plus m-r-5"></i> Mogok</a>
				@endif
			</div>
		...
		<div class="card-block table-responsive">
			<table class="table table-hover " id="table-strikes">
				<thead>
					<tr>
						<th class="fit">Bil.</th>
						<th>Nama Kesatuan</th>
		...
		<div class="modal-content-wrapper">
			<div class="modal-content">
				...
				<div class="modal-footer" style="background-color: #f3f3f3;">
                    ...
                    <a href="{{ route('strike') }}" class="btn btn-primary mt-4"><i class="fa fa-edit mr-1 text-capitalize"></i> Kemaskini Borang</a>
                    @endif
...
function process(id) {
    $("#modal-div").load("{{ route('strike') }}/"+id+"/process/result");
}

function receive(id) {
    $("#modal-div").load("{{ route('strike') }}/"+id+"/process/document-receive");
}

function query(id) {
    $("#modal-div").load("{{ route('strike') }}/"+id+"/process/query");
}

function recommend(id) {
    $("#modal-div").load("{{ route('strike') }}/"+id+"/process/recommend");
}

function status(id) {
    $("#modal-div").load("{{ route('strike') }}/"+id+"/process/status");
}
</script>
@endpush
