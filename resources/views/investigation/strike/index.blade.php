				...
				<div class="col-xl-12 col-lg-12 ">
					...
					<div class="card card-transparent">
						<div class="card-block p-t-0">
							<h3 class='m-t-0'>Pengendalian Mogok / Tutup Pintu</h3>
    ...
    <div class="row justify-content-center">
        ...
        <div class="col-lg-12">
            @component('components.block', [
                    'percentage' => (11 - $errors_strike)/11*100,
                    'title' => 'Permohonan Mogok',
                    'description' => '',
                ])
                <div class="btn-group-custom">
                    <a href="{{ route('strike.index', $strike->id) }}" class="btn btn-primary pull-right btn-input"><i class="fa fa-edit mr-1"></i> Isi Borang</a>
                    @if(!$errors_strike)
                    <a href="{{ route('download.strike', $strike->id) }}" target="_blank" class="btn btn-default mr-2 pull-right btn-lock"><i class="fa fa-print mr-1"></i> Cetak</a>
        ...
        <div class="col-lg-12">
            @component('components.block', [
                ...
                <div class="btn-group-custom">
                    <a href="{{ route('strike.formu', $strike->id) }}" class="btn btn-primary pull-right btn-input"><i class="fa fa-edit mr-1"></i> Isi Borang</a>
                </div>
                @if(!$errors_u)
        ...
        <div class="col-lg-12">
            @component('components.block', [
                ...
                <div class="btn-group-custom">
                    <a href="{{ url('files/checklist/strike.pdf') }}" target="_blank" class="btn btn-default mr-2 pull-right"><i class="fa fa-print mr-1"></i> Cetak</a>
                </div>
                <hr>
                <ul>
                    <li><label>Surat Pemberitahuan Mogok (2 salinan)</label></li>
                    <li><label>Buku Perlembagaan Tanpa Cawangan / Buku Perlembagaan Bercawangan (4 Salinan Asal)</label></li>
                    <li><label>Usul Dan Minit Mesyuarat Keputusan Mengadakan Mogok (2 salinan)</label></li>
        ...
        <div class="col-lg-12">
            @component('components.block', [
                ...
                <div class="row">
                    <div class="col-md-12">
                        <form action="{{ route('strike.form.attachment', $strike->id) }}" enctype="multipart/form-data" class="attachment dropzone no-margin">
                            <div class="fallback">
        ...
        @if($strike->created_by_user_id == auth()->id())
...
<script type="text/javascript">
    ...
    @if(!request()->id)
        window.history.pushState('strike', 'Pengendalian Mogok', '{{ fullUrl() }}/{{ $strike->id }}');
    @endif
    ...
    function submit() {
        ...
        .then((confirm) => {
            if (confirm) {
                $.ajax({
                    url: '{{ route("strike.form", $strike->id) }}',
                    method: 'POST',
                    dataType: 'json',
                    async: true,
                    contentType: false,
                    ...
                    success: function(data) {
                        if(data.status == 'error') {
                            ...
                            .then((confirm) => {
                                if (confirm) {
                                    location.href="{{ route('strike.list') }}";
                                }
                            });
                        }
                    }
    ...
    $(".attachment").dropzone({
        ...
        url: "{{ route('strike.form.attachment', $strike->id) }}",
        addRemoveLinks : true,
        ...
        removedfile: function(file) {
            ...
            .then((confirm) => {
                if (confirm) {
                    $.ajax({
                        url: '{{ route('strike.form.attachment', $strike->id) }}/'+file.previewElement.id,
                        method: 'delete',
                        dataType: 'json',
                        async: true,
                        contentType: false,
