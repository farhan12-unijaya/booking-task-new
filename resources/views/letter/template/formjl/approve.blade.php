...
@include('components.bs.textarea', [
	...
	'label' => 'Alamat Firma',
	'mode' => 'disabled',
	'value' => $letter->filing->firm_address->address1.
	    ($letter->filing->firm_address->address2 ? ', '.$letter->filing->firm_address->address2 : '').
	    ($letter->filing->firm_address->address3 ? ', '.$letter->filing->firm_address->address3 : '').
	    ', '.
	    $letter->filing->firm_address->postcode.' '.
	    ($letter->filing->firm_address->district ? $letter->filing->firm_address->district->name : '').', '.
	    ($letter->filing->firm_address->state ? $letter->filing->firm_address->state->name : '')
])
...
@include('components.bs.date', [
	'name' => 'reply_date',
	'label' => 'Tarikh Akhir Maklumbalas',
    'value' => array_key_exists('reply_date',$data) ? $data['reply_date'] : date('d/m/Y'),
])
