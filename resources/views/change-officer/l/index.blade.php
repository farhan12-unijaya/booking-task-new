				...
				<div class="col-xl-12 col-lg-12 ">
					...
					<div class="card card-transparent">
						<div class="card-block p-t-0">
							<h3 class='m-t-0'>Perubahan <span class="semi-bold">Pegawai Kesatuan Sekerja</span></h3>
							...
							<div class="row">
								...
								@if($forml->tenure->entity->branches)
								<div class="col-md-4 col-sm-6">
									<div class="form-group form-group-default form-group-default-select2 required" aria-required="true">
										...
										<select id="branch_id" name="branch_id" class="full-width autoscroll" data-init-plugin="select2" required>
											<option value="-1" selected>Induk - {{ $forml->tenure->entity->name }}</option>
											@foreach($forml->tenure->entity->branches as $branch)
	...
	<div class="row">
		<div class="col-lg-12">
            @component('components.block', [
                    'percentage' => (2 - count($error_list['l']))/2*100,
                    'title' => 'Borang L',
                    'description' => '- Notis Perubahan Pegawai-Pegawai Atau Titel Pegawai-Pegawai',
                ])
                <div class="btn-group-custom">
                    <a href="{{ route('forml.form', $forml->id) }}" class="btn btn-primary pull-right btn-input"><i class="fa fa-edit mr-1"></i> Isi Borang</a>
                    @if(!count($error_list['l']))
                    <a href="{{ route('download.forml', $forml->id) }}" target="_blank" class="btn btn-default mr-2 pull-right btn-lock"><i class="fa fa-print mr-1"></i> Cetak</a>
        ...
        <div class="col-lg-12">
            @component('components.block', [
                ...
                <div class="btn-group-custom">
                    <a href="{{ route('forml.praecipe', $forml->id) }}" target="_blank" class="btn btn-default mr-2 pull-right"><i class="fa fa-print mr-1"></i> Cetak</a>
                </div>
            @endcomponent
        </div>
        @if($forml->created_by_user_id == auth()->id())
...
@if(!request()->id)
	window.history.pushState('forml', 'Borang L', '{{ fullUrl() }}/{{ $forml->id }}');
@endif
			...
			$.ajax({
				...
				success: function(data) {
					if(data.status == 'error') {
						...
						.then((confirm) => {
							if (confirm) {
								location.href="{{ route('forml.list') }}";
							}
						});
					}
				}
...
$(document).ready(function(){
	...
	@if($forml->tenure_id)
        $("#tenure_id").val({{ $forml->tenure_id }}).trigger('change');
    @endif

    @if($forml->branch_id)
        $("#branch_id").val({{ $forml->branch_id }}).trigger('change');
    @endif
});
