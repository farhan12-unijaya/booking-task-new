		...
		<div class="modal-content-wrapper">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="form-add-leaving" role="form" method="post" action="{{ route('formlu.leaving', $formlu->id) }}">
...
var settings = {
	...
	"serverSide": true,
	"deferRender": true,
	"ajax": "{{ route('formlu.leaving', $formlu->id) }}",
	"columns": [
...
function editLeaving(id) {
	$("#modal-div").load("{{ route('formlu.leaving', $formlu->id) }}/"+id);
}
...
function removeLeaving(id) {
	...
	.then((confirm) => {
		if (confirm) {
			$.ajax({
				url: '{{ route('formlu.leaving', $formlu->id) }}/'+id,
				method: 'delete',
				dataType: 'json',
				async: true,
				contentType: false,
