...
<div id="rootwizard">
	...
	<ul class="nav nav-tabs nav-tabs-linetriangle nav-tabs-separator nav-stack-sm" role="tablist" {{-- data-init-reponsive-tabs="dropdownfx" --}} >
		...
		<li class="nav-item">
			<a class="" data-toggle="tab" href="#" data-target="#tab2" role="tab"><i class="fa tab-icon {{ $formlu->leaving->count() < 1 ? 'fa-times text-danger' : 'fa-check text-success'}}"></i> <span>Butiran Pegawai Yang Meninggalkan Jawatan</span></a>
		</li>
		<li class="nav-item">
			<a class="" data-toggle="tab" href="#" data-target="#tab3" role="tab"><i class="fa tab-icon {{ $formlu->officers->count() < 1 ? 'fa-times text-danger' : 'fa-check text-success'}}"></i> <span>Butiran Pegawai Yang Memegang Jawatan</span></a>
		</li>
	</ul>
	...
	<div class="tab-content">
		<div class="tab-pane active slide-right" id="tab1">
			@include('change-officer.lu.forml.tab1')
		</div>
		<div class="tab-pane slide-right" id="tab2">
			@include('change-officer.lu.forml.tab2.index')
		</div>
		<div class="tab-pane slide-right" id="tab3">
			@include('change-officer.lu.forml.tab3.index')
		</div>
	</div>
</div>
@endsection
...
///////////////////////////////////////////////////////////////////////////////////
var required = {
	'resolved_at': '{{ $formlu->resolved_at ? 1 : 0 }}',
	'meeting_type_id': '{{ $formlu->meeting_type_id ? 1 : 0 }}',
}
...
$(document).ready(function(){
	...
	$("#has_branch_{{ isset($formlu->has_branch) ? $formlu->has_branch ? 'yes' : 'no' : '' }}").prop('checked', true).trigger('change');;

	$("#meeting_type_{{ $formlu->meeting_type_id ? $formlu->meeting_type_id : '' }}").prop('checked', true).trigger('change');;
