				...
				<div class="col-xl-12 col-lg-12 ">
					...
					<div class="card card-transparent">
						<div class="card-block p-t-0">
							<h3 class='m-t-0'>Senarai <span class="semi-bold">Pendaftaran Perubahan Pegawai / Pekerja Kesatuan Sekerja</span></h3>
							<p class="small hint-text m-t-5">
								Pengurusan pendaftaran perubahan pegawai / pekerja kesatuan sekerja boleh dilakukan melalui jadual di bawah
							</p>
						</div>
					</div>
					<!-- END card -->
		...
		<div class="card-header px-0">
			<div class="card-title">
				@if(auth()->user()->hasAnyRole(['ks']))
					<button onclick="location.href='{{ route('formlu') }}'" class="btn btn-success btn-cons"><i class="fa fa-plus m-r-5"></i> Permohonan</button>
				@endif
            </div>
		...
		<div class="modal-content-wrapper">
			<div class="modal-content">
				...
				<div class="modal-footer" style="background-color: #f3f3f3;">
                    ...
                    <a href="{{ route('formlu') }}" class="btn btn-primary mt-4"><i class="fa fa-edit mr-1 text-capitalize"></i> Kemaskini Borang</a>
                    @endif
...
function process(id) {
    $("#modal-div").load("{{ route('formlu') }}/"+id+"/process/result");
}

function receive(id) {
    $("#modal-div").load("{{ route('formlu') }}/"+id+"/process/document-receive");
}

function query(id) {
    $("#modal-div").load("{{ route('formlu') }}/"+id+"/process/query");
}

function recommend(id) {
    $("#modal-div").load("{{ route('formlu') }}/"+id+"/process/recommend");
}
</script>
@endpush
