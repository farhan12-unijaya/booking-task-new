<?php
$industries = array(
    'Pertanian, Perhutanan dan Perikanan',
    'Perlombongan dan Pengkuarian',
    'Pembuatan',
    'Bekalan Elektrik, Gas, Wap dan Pendingin Udara',
    'Bekalan Air, Pembentungan, Pengurusan Sisa dan Aktiviti Pemulihan',
    'Pembinaan',
    'Perdagangan Jual Borong dan Jual Runcit, Pembaikan Kenderaan Bermotor dan Motosikal',
    'Pengangkutan dan Penyimpanan',
    'Penginapan dan Aktiviti Perkhidmatan Makanan Dan Minuman',
    'Maklumat dan Komunikasi',
    'Aktiviti Kewangan dan Insuran/Takaful',
    'Aktiviti Hartanah',
    'Aktiviti Profesional, Saintifik dan Teknikal',
    'Aktiviti Pentadbiran dan Khidmat Sokongan',
    'Pentadbiran Awam dan Pertahanan; Aktiviti Keselamatan Sosial Wajib',
    'Pendidikan',
    'Kesihatan dan Kerja Sosial',
    'Kesenian, Hiburan dan Rekreasi',
    'Aktiviti Perkhidmatan Lain',
    'Aktiviti Isi Rumah Sebagai Majikan; Aktiviti Mengeluarkan Barangan dan Perkhidmatan Yang Tidak Dapat Dibezakan Oleh Isi Rumah Untuk Kegunaan Sendiri',
    'Aktiviti Badan dan Pertubuhan Luar Wilayah',
);
?>
<div class="tab-pane active slide-right" id="tab1">
	@include('report.public.17.tab1')
</div>
<div class="tab-pane slide-right" id="tab2">
	@include('report.public.17.tab2')
</div>
