...
<div class="jumbotron" data-pages="parallax">
    <div class=" container-fluid container-fixed-lg sm-p-l-0 sm-p-r-0">
        <div class="inner">
            <!-- START BREADCRUMB -->
            {{ Breadcrumbs::render('admin.master.room') }}
            <!-- END BREADCRUMB -->
            <div class="row">
                <div class="col-xl-12 col-lg-12 ">
                    ...
                    <div class="card card-transparent">
                        <div class="card-block p-t-0">
                            <h3 class='m-t-0'>Data Induk - Ruangan</h3>
                            <p class="small hint-text m-t-5">
                                Pengurusan data induk (Ruangan) boleh dilakukan melalui jadual di bawah.
                            </p>
                        </div>
                    </div>
                    <!-- END card -->
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addModalTitle">Tambah <span class="bold">Ruangan</span></h5>
            ...
            <div class="modal-body m-t-20">
                <form id='form-add' role="form" method="post" action="{{ route('admin.master.room') }}">
...
                            <select name="master_room_category_id" id="holiday_type_id" class="full-width autoscroll" data-init-plugin="select2" required="">
...
                    </div>
                    ...
                    @include('components.input', [
                        'name' => 'price_non_holiday',
                        'label' => 'Harga Non Holiday',
                        'mode' => 'required',
                        'value' => ''
                    ])
                    @include('components.input', [
...
$("#form-add").submit(function(e) {
    ...
    if(!form.valid())
       return;
    console.log('tesss', form.attr('action'));
    ...
    $.ajax({
        ...
        success: function(data) {
            console.log(data, 'tezzz');
            swal(data.title, data.message, data.status);
            $("#modal-add").modal("hide");
            table.api().ajax.reload(null, false);
        },
        error: function(json){
			alert('ada error', json.responseJSON);
		}
    });
});
...
function remove(id) {
    ...
    .then((confirm) => {
        if (confirm) {
            $.ajax({
                url: '{{ route('admin.master.room-category') }}/'+id,
                method: 'delete',
                dataType: 'json',
                async: true,
                contentType: false,
