        ...
        <div class="modal-content-wrapper">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="form-add-examiner" role="form" method="post" action="{{ route('formk.k2.form.examiner', [$formk->id, $formu->id]) }}">
...
var settings = {
    ...
    "serverSide": true,
    "deferRender": true,
    "ajax": "{{ route('formk.k2.form.examiner', [$formk->id, $formu->id]) }}",
    "columns": [
...
function editExaminer(id) {
    $("#modal-div").load("{{ route('formk.k2.form.examiner', [$formk->id, $formu->id]) }}/"+id);
}
...
function removeExaminer(id) {
    ...
    .then((confirm) => {
        if (confirm) {
            $.ajax({
                url: '{{ route('formk.k2.form.examiner', [$formk->id, $formu->id]) }}/'+id,
                method: 'delete',
                dataType: 'json',
                async: true,
                contentType: false,
