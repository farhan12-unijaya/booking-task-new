                ...
                <div class="col-xl-12 col-lg-12 ">
                    ...
                    <div class="card card-transparent">
                        <div class="card-block p-t-0">
                            <h3 class='m-t-0'>Senarai <span class="semi-bold">Borang J</span></h3>
                            <p class="small hint-text m-t-5">
                                Pengurusan permohonan Borang J boleh dilakukan melalui jadual di bawah.
                            </p>
                        </div>
                    </div>
                    <!-- END card -->
                ...
                @if(auth()->user()->hasAnyRole(['ks']))
                    @if(auth()->user()->entity->tenures->count() > 1)
                        ...
                        @if($tenure->formlu()->where('filing_status_id', 9)->count() == 0)
                            <button onclick="swal('Harap Maaf!', 'Sila mendaftar borang LU bagi penggal ini terlebih dahulu atau hubungi admin.', 'error')" class="btn btn-success btn-cons"><i class="fa fa-plus m-r-5"></i> Borang J</button>
                        @else
                            <button onclick="location.href='{{ route('formj') }}'" class="btn btn-success btn-cons"><i class="fa fa-plus m-r-5"></i> Borang J</button>
                        @endif
                    @else
                        <button onclick="location.href='{{ route('formj') }}'" class="btn btn-success btn-cons"><i class="fa fa-plus m-r-5"></i> Borang J</button>
                    @endif
            ...
            <table class="table table-hover" id="table">
                <thead>
                    <tr>
                        ...
                        <th>Daerah</th>
                        <th>Negeri</th>
                        <th class="fit">Tarikh Permohonan</th>
                        <th class="fit">Status</th>
                        <th class="fit">Surat / Lampiran</th>
                        <th>Tindakan</th>
                    </tr>
                </thead>
            </table>
        ...
        <div class="modal-content-wrapper">
            <div class="modal-content">
                ...
                <div class="modal-footer" style="background-color: #f3f3f3;">
                    ...
                    @if(auth()->user()->hasAnyRole(['ks']))
                    <a href="{{ route('formk') }}" class="btn btn-success mt-4"><i class="fa fa-plus mr-1 text-capitalize"></i>Borang K</a>
                    @endif
                    @if(auth()->user()->hasAnyRole(['ks', 'ppw', 'pphq']))
                    <a href="{{ route('formj') }}" class="btn btn-primary mt-4"><i class="fa fa-edit mr-1 text-capitalize"></i> Kemaskini Borang J</a>
                    @endif
                    @if(auth()->user()->hasAnyRole(['ks', 'ppw', 'pphq']))
                    <a href="{{ route('formk') }}" class="btn btn-primary mt-4"><i class="fa fa-edit mr-1 text-capitalize"></i> Kemaskini Borang K</a>
                    @endif
                    ...
                    @endif
                    @if(auth()->user()->hasAnyRole(['pw','pkpg','pkpp']))
                    <button onclick="editAcceptData(1)" class="btn btn-primary mt-4"><i class="fa fa-check mr-1 text-capitalize"></i> Kemaskini Tarikh Terima Dokumen</button>
                    @endif
...
function process(id) {
    $("#modal-div").load("{{ route('formj') }}/"+id+"/process/result");
}

function receive(id) {
    $("#modal-div").load("{{ route('formj') }}/"+id+"/process/document-receive");
}

function query(id) {
    $("#modal-div").load("{{ route('formj') }}/"+id+"/process/query");
}

function recommend(id) {
    $("#modal-div").load("{{ route('formj') }}/"+id+"/process/recommend");
}

function status(id) {
    $("#modal-div").load("{{ route('formj') }}/"+id+"/process/status");
}
