                ...
                <div class="col-xl-12 col-lg-12 ">
                    ...
                    <div class="card card-transparent">
                        <div class="card-block p-t-0">
                            <h3 class='m-t-0'>Senarai <span class="semi-bold">Borang G</span></h3>
                            <p class="small hint-text m-t-5">
                                Pengurusan permohonan Borang G boleh dilakukan melalui jadual di bawah.
                            </p>
                        </div>
                    </div>
                    <!-- END card -->
                ...
                @if(auth()->user()->hasAnyRole(['ks']))
                    @if(auth()->user()->entity->tenures->count() > 1)
                        ...
                        @if($tenure->formlu()->where('filing_status_id', 9)->count() == 0)
                            <button onclick="swal('Harap Maaf!', 'Sila mendaftar borang LU bagi penggal ini terlebih dahulu atau hubungi admin.', 'error')" class="btn btn-success btn-cons"><i class="fa fa-plus m-r-5"></i> Borang G</button>
                        @else
                            <button onclick="location.href='{{ route('formg') }}'" class="btn btn-success btn-cons"><i class="fa fa-plus m-r-5"></i> Borang G</button>
                        @endif
                    @else
                        <button onclick="location.href='{{ route('formg') }}'" class="btn btn-success btn-cons"><i class="fa fa-plus m-r-5"></i> Borang G</button>
                    @endif
        ...
        <div class="modal-content-wrapper">
            <div class="modal-content">
                ...
                <div class="modal-footer" style="background-color: #f3f3f3;">
                    ...
                    <a href="{{ route('formg') }}" class="btn btn-primary mt-4"><i class="fa fa-edit mr-1 text-capitalize"></i> Kemaskini Borang G</a>
                    @endif
                    @if(auth()->user()->hasAnyRole([ 'ks', 'ppw', 'pphq']))
...
function process(id) {
    $("#modal-div").load("{{ route('formg') }}/"+id+"/process/result");
}

function receive(id) {
    $("#modal-div").load("{{ route('formg') }}/"+id+"/process/document-receive");
}

function query(id) {
    $("#modal-div").load("{{ route('formg') }}/"+id+"/process/query");
}

function recommend(id) {
    $("#modal-div").load("{{ route('formg') }}/"+id+"/process/recommend");
}

function status(id) {
    $("#modal-div").load("{{ route('formg') }}/"+id+"/process/status");
}
