...
class FormIController extends Controller
{
    //
    /**
    ...
    public function index(Request $request) {
        ...
        $formi = $dissolution->formi;
        $meeting_types = MasterMeetingType::whereIn('id', [2,3])->get();
        $states = MasterState::all();

    	return view('dissolution-cancellation.dissolution.formi.index', compact('dissolution', 'formi', 'meeting_types', 'states'));
    }
    ...
    public function member_index(Request $request) {
        ...
        $log->description = "Papar senarai Borang I - Butiran Ahli";
        ...
        $dissolution = FormIEU::findOrFail($request->id);
        $formi = $dissolution->formi;
        $members = $formi->members()->with(['address'])->get();
    ...
    public function member_insert(Request $request) {
        ...
        $dissolution = FormIEU::findOrFail($request->id);
        $formi = $dissolution->formi;
        $member = $formi->members()->create([
            'name' => $request->name,
            'address_id' => $address->id,
        ]);

        $count = $formi->members->count();

        $log = new LogSystem;
        $log->module_id = 27;
        $log->activity_type_id = 4;
        $log->description = "Tambah Borang I - Butiran Ahli";
    ...
    public function member_edit(Request $request) {
        ...
        $log->description = "Popup kemaskini Borang I - Butiran Ahli";
        $log->url = $request->fullUrl();
        $log->method = strtoupper($request->method());
        $log->ip_address = $request->ip();
        $log->created_by_user_id = auth()->id();
        ...
        return view('dissolution-cancellation.dissolution.formi.tab2.edit', compact('dissolution', 'formi', 'member', 'states'));
    }
    ...
    public function member_update(Request $request) {
        ...
        $log->description = "Kemaskini Borang I - Butiran Ahli";
    ...
    public function member_delete(Request $request) {
        ...
        $log->description = "Padam Borang I - Butiran Ahli";
        $log->data_old = json_encode($member);
        $log->url = $request->fullUrl();
        $log->method = strtoupper($request->method());
        $log->ip_address = $request->ip();
    ...
    //Member CRUD END
        ...
        $data = [                                                                                       // Change here
            ...
            'concluded_day' => htmlspecialchars(strftime('%e', strtotime($filing->concluded_at))),
            'concluded_month_year' =>  htmlspecialchars(strftime('%B %Y', strtotime($filing->concluded_at))),
        ...
        $document = $phpWord->loadTemplate(storage_path('templates/filings/dissolution-cancellation/formi.docx'));        // Change here
        ...
        $file_name = uniqid().'_'.'Borang I';                                                          // Change here
